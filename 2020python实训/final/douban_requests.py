...
class DouBanRequest(object):
    ...
    def get_movie_short_comments(self, movie_id='27668550', page=1, sort='new_score', status='P', percent_type='',
        ...
        :param movie_id: 电影ID
        :param page: 页面数，需要处理为开始条数（每页显示20条数据）
        :param sort: 排序规则，热门：new_score； 最新：time
        :param status: 状态，看过：P; 想看：F
        :param percent_type: 选定好评中评差评（状态必须为P）,好评：h; 中评：m; 差评：l
        :return:
        '''
        # 处理开始条数
        start = (page - 1) * 20
